...
class MultiheadAttention(nn.Module):
    def __init__(self, key_size, query_size, value_size, num_hiddens,
        ...
        self.W_q = nn.Linear(query_size, bias=bias)
        self.W_k = nn.Linear(key_size, bias=bias)
        self.W_v = nn.Linear(value_size, bias=bias)
...
class TransformerEncoder(Encoder):
    ...
    def forward(self, X, valid_lens, *args):
        ...
        for i, blk in enumerate(self.blks):
            X = blk(X, valid_lens)
            self.attention_weights[i] = blk.attention.attention.attention.weight
        return X    
...
if __name__ == '__main__':
    ...
    key_size, query_size, value_size = 32, 64, 32
    norm_shape = [32]
